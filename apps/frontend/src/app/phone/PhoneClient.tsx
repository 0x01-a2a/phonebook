...
export default function PhoneClient() {
  ...
  const statusColor = browserState === 'connected' ? PX.green : browserState === 'connecting' || browserState === 'requesting_mic' ? PX.blue : browserState === 'error' ? PX.red : PX.grayLight;
  const statusText = browserState === 'idle' ? 'READY' : browserState === 'requesting_mic' ? 'MIC ACCESS...' : browserState === 'connecting' ? 'CONNECTING...' : browserState === 'connected' ? `LIVE ${formatDuration(callDuration)}` : browserState === 'ended' ? `ENDED ${formatDuration(callDuration)}` : 'ERROR';
  ...
  const phonePanel = (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem', maxWidth: 400, margin: '0 auto', width: '100%' }}>
      ...
      {isActive && selectedAgent && (
        <div style={{ width: '100%', background: PX.black, padding: '1.5rem 1rem', ...pixelBorder(PX.green, 3), textAlign: 'center' }}>
          <div style={{
            width: 64, height: 64, borderRadius: '50%',
            background: browserState === 'connected' ? `radial-gradient(circle, ${PX.green}, ${PX.greenDark})` : `radial-gradient(circle, ${PX.blue}, ${PX.blueDark})`,
            margin: '0 auto 16px',
          ...
          <div style={{ fontFamily: 'var(--font-pixel)', fontSize: '0.35rem', color: statusColor, letterSpacing: '0.1em', marginBottom: 16 }}>{statusText}</div>
